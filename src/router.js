...
router.get("/clear-bindings", (req, res) => {
  clearBindings().then(data => {
    res.status(data.status).send(data.data);
  });
});
...
router.get("/messenger_code", function(req, res) {
  ...
  request.post(
    "https://graph.facebook.com/v2.6/me/messenger_codes?access_token=" +
  ...
  if (message.sender) {
    ...
    request.post(
        ...
        process.env.FB_PAGE_ACCESS_TOKEN,
      {
        ...
        body: {
          recipient: {
            user_ref: message.optin.user_ref
          },
          message: {
      ...
      (err, resp, body) => {
        if (err) {
          console.error(error);
          res.status(502).send(resp.statusMessage + ": " + error);
        } else {
          console.log(resp.statusMessage);
          res.status(200).send(resp.statusMessage);
        }
      }
    );
  }
